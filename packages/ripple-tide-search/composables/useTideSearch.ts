...
export default ({
  ...
  const getSearchResults = async (isFirstRun: boolean) => {
    ...
    try {
      ...
      // const asdasd = Object.keys(searchResponse.aggregations).reduce(
      //   (aggs, key) => {
      //     return {
      //       ...aggs,
      //       [`${key}`]: searchResponse.aggregations[key].buckets.map(
      //         (bkt) => bkt
      //       )
      //     }
      //   },
      //   {}
      // )
      // onAggregationUpdateHook.value(asdasd)
