...
export default ({
  ...
  const getQueryClause = () => {
    if (searchTerm.value.q) {
      const fns: Record<string, (queryValues: any) => Record<string, any>> =
        appConfig?.ripple?.search?.queryConfigFunctions || {}

      if (typeof queryConfig !== 'string') {
        return processTemplate(queryConfig, '{{query}}', searchTerm.value.q)
      } else if (fns[queryConfig]) {
        const conf = fns[queryConfig](searchTerm.value)
