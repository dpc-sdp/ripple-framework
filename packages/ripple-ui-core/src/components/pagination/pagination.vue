...
<template>
  <nav :class="`rpl-pagination rpl-pagination--${variant}`" :aria-label="label">
    <p v-if="showTally && !isComplex" class="rpl-pagination__tally rpl-type-label">
      {{ activeStep }} of {{ totalPages }}
    </p>
    <RplPaginationLink v-if="!isComplex || !isFirstStep" icon-name="icon-arrow-left" :icon-size="iconSize"
      :aria-label="`Go to previous ${contentType}`" :disabled="!isComplex && isFirstStep"
      @click="(e) => onClick(e, activeStep - 1)">
      Previous
    </RplPaginationLink>
    <ol v-if="isComplex" class="rpl-pagination__list">
      <li v-for="(step, index) in visibleSteps" :key="index" class="rpl-pagination__list-item rpl-type-label">
        <button v-if="step" class="rpl-pagination__page rpl-u-focusable-block"
          :aria-label="`Go to ${contentType} ${step}`" :aria-current="step === activeStep ? true : null"
    ...
    <RplPaginationLink v-if="!isComplex || !isLastStep" icon-name="icon-arrow-right" icon-placement="after"
      :icon-size="iconSize" :aria-label="`Go to next ${contentType}`" :disabled="!isComplex && isLastStep"
      @click="(e) => onClick(e, activeStep + 1)">
      Next
    </RplPaginationLink>
  </nav>
